...
void Application::ProcessInput() {
    ...
    bool b_pressed = (glfwGetKey(glfw_window, GLFW_KEY_B) == GLFW_PRESS);
    
    if (b_pressed && !b_was_pressed && !camera_enabled_) {
        motion_blur_enabled_ = !motion_blur_enabled_;
        if (motion_blur_enabled_) {
            grassland::LogInfo("Motion Blur enabled");
        } else {
            grassland::LogInfo("Motion Blur disabled");
        }
        film_->Reset();
        frame_count_ = 0;
    }
    ...
    bool d_pressed = (glfwGetKey(glfw_window, GLFW_KEY_D) == GLFW_PRESS);
    
    if (d_pressed && !d_was_pressed && !camera_enabled_) {
        dispersion_enabled_ = !dispersion_enabled_;
        if (dispersion_enabled_) {
            grassland::LogInfo("Dispersion enabled");
        } else {
            grassland::LogInfo("Dispersion disabled");
        }
        film_->Reset();
        frame_count_ = 0;
    }
...
void Application::ApplyDepthOfFieldParams() {
    ...
    if (temp_aperture_size_ < 0.001f || temp_aperture_size_ > 5.0f) {
        grassland::LogWarning("Aperture must be between 0.001 and 5m");
        valid = false;
    }
    
    if (temp_focal_length_ < 0.01f || temp_focal_length_ > 2.0f) {
...
void Application::ApplyMotionBlurParams() {
    ...
    if (temp_exposure_time_ < 0.000f || temp_exposure_time_ > 2.0f) {
        grassland::LogWarning("Exposure time must be between 0.000 and 2.0 seconds");
        valid = false;
    }
    
    if (glm::length(temp_camera_linear_velocity_) > 100.0f) {
        grassland::LogWarning("Camera linear velocity too high (max 100 m/s)");
        valid = false;
    }
    
    if (std::abs(temp_camera_angular_velocity_) > 10.0f) {
        grassland::LogWarning("Camera angular velocity too high (max 10 rad/s)");
        valid = false;
    }
    ...
    grassland::LogInfo("Motion blur parameters applied:");
    grassland::LogInfo("  Exposure time: {:.3f}s", exposure_time_);
    grassland::LogInfo("  Camera linear velocity: ({:.2f}, {:.2f}, {:.2f}) m/s", 
                      camera_linear_velocity_.x, camera_linear_velocity_.y, camera_linear_velocity_.z);
...
void Application::OnInit() {
    ...
    initial_settings.max_depth = MAX_DEPTH;
    ...
    initial_settings.resolution = glm::vec2((float)window_->GetWidth(), (float)window_->GetHeight());
...
void Application::OnUpdate() {
    ...
    if (alive_) {
        ...
        camera_object.exposure_time = exposure_time_;
        camera_object.enable_dispersion = (dispersion_enabled_ && !camera_enabled_) ? 1 : 0;
...
void Application::RenderInfoOverlay() {
    ...
    if (depth_of_field_ui_open_) {
        ...
        if (depth_of_field_enabled_) {
            ...
            } else if (temp_aperture_size_ < 0.001f || temp_aperture_size_ > 5.0f) {
                params_valid = false;
                error_msg = "Aperture must be between 0.001 and 5m";
            } else if (temp_focal_length_ < 0.01f || temp_focal_length_ > 2.0f) {
                params_valid = false;
    ...
    ImGui::TextColored(ImVec4(0.5f, 1.0f, 0.5f, 1.0f), "B Key: Toggle Motion Blur");
    
    if (ImGui::Button(motion_blur_ui_open_ ? "Hide Motion Blur Settings" : "Show Motion Blur Settings")) {
        motion_blur_ui_open_ = !motion_blur_ui_open_;
        if (motion_blur_ui_open_) {
            temp_exposure_time_ = exposure_time_;
            temp_camera_linear_velocity_ = camera_linear_velocity_;
            temp_camera_angular_velocity_ = camera_angular_velocity_;
        }
    }
    ...
    if (motion_blur_ui_open_) {
        ...
        if (motion_blur_enabled_) {
            ...
            ImGui::Text("  Linear Vel: (%.2f, %.2f, %.2f) m/s", 
                       camera_linear_velocity_.x, camera_linear_velocity_.y, camera_linear_velocity_.z);
            ImGui::Text("  Angular Vel: %.2f deg/s", glm::degrees(camera_angular_velocity_));
            ...
            ImGui::SameLine();
            ImGui::PushItemWidth(120.0f);
            ImGui::InputFloat("##temp_exposure_time", &temp_exposure_time_, 0.0f, 0.0f, "%.3f");
            ImGui::PopItemWidth();
            ImGui::SameLine();
            ...
            if (temp_exposure_time_ < 0.0f || temp_exposure_time_ > 2.0f) {
                params_valid = false;
                error_msg = "Exposure time must be between 0.00 and 2.00 seconds";
            } else if (glm::length(temp_camera_linear_velocity_) > 100.0f) {
                params_valid = false;
                error_msg = "Camera linear velocity too high (max 100 m/s)";
            } else if (std::abs(temp_camera_angular_velocity_) > 10.0f) {
                params_valid = false;
    ...
    bool disp_enabled = dispersion_enabled_;
    if (ImGui::Checkbox("Enable Dispersion", &disp_enabled)) {
        dispersion_enabled_ = disp_enabled;
        if (!camera_enabled_) {
            film_->Reset();
            frame_count_ = 0;
        }
...
void Application::OnRender() {
    ...
    // motion blur
    command_context->CmdBindResources(14, { scene_->GetMotionGroupsBuffer() }, grassland::graphics::BIND_POINT_RAYTRACING);
    ...
    if ((int)mip_images.size() < maxMipImages) {
        ...
        for (int i = 0; i < pad; ++i) {
            mip_images.push_back(dummyImage.get());
        }
    } else if ((int)mip_images.size() > maxMipImages) {
        grassland::LogWarning("Mip atlas images exceed the binding limit ({}). Some levels will be dropped.", maxMipImages);
        mip_images.resize(maxMipImages);
    }
    ...
    command_context->CmdBindResources(23, { media_buffer_.get() }, grassland::graphics::BIND_POINT_RAYTRACING);
